import { SUCCESS_CATEGORY_CREATED, SUCCESS_CATEGORY_UPDATED, SUCCESS_GET_ALL, SUCCESS_CATEGORY_DELETED } from "../constants/messages.constant.js";
import { ERROR_CATEGORY_NOT_FOUND, ERROR_CREATE_CATEGORY, ERROR_DELETE_CATEGORY, ERROR_QUERY, ERROR_UPDATE_CATEGORY } from "../constants/errors.constant.js";
    ...
    connection.query(query, [category.id], (err, result) => {
        if(!err){
            if(result.affectedRows == 0){
                ...
                return res.status(200).send({message: SUCCESS_CATEGORY_DELETED, payload: result})
            }
        } else{
            return res.status(500).send({message: ERROR_DELETE_CATEGORY, error: err})
        }
    })
}